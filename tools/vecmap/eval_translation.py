...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate embeddings of two languag    es in a shared space in word translation induction')
    ...
    parser.add_argument('--p_at', default=1, type=int, help='precision at x')
    parser.add_argument('--lower', action='store_true', help='lowercase the embedding words')
    ...
    # print("Big  line:", len(src_words), x.shape, len(trg_words), z.shape)
    ...
    if args.retrieval == 'nn':  # Standard nearest neighbor
        for i in range(0, len(src), BATCH_SIZE):
            ...
            if args.p_at > 1:
                # nn = similarities.argpartition(-args.p_at, axis=1)[:, -args.p_at:]
                nn = (-similarities).argsort(axis=1)[:, :args.p_at]

                a = similarities.argmax(axis=1)
                b = nn[:,:1].flatten()
                assert (a==b).all(), str(a) + "\n" + str(b) + "\n" + str(nn) + "\n" + str(similarities)
                for k in range(j-i):
    ...
    if args.p_at == 1:
        ...
        total = 0.0
        for i in src:
            # print(translation[i], src2trg[i])
            if int(translation[i]) in src2trg[i]:
                correct+=1
            total +=1
        ...
        print('Coverage:{0:7.2%}  Accuracy:{1:7.2%}'.format(coverage, correct/total))
    else:
        calc_prec(src, translation, src2trg, coverage, 1, args.lower, src_words, trg_words)
        if args.p_at > 5:
            calc_prec(src, translation, src2trg, coverage, 5, args.lower, src_words, trg_words)
        if args.p_at > 10:    
            calc_prec(src, translation, src2trg, coverage, 10, args.lower, src_words, trg_words)
        calc_prec(src, translation, src2trg, coverage, args.p_at, args.lower, src_words, trg_words)

def calc_prec(src, translation, src2trg, coverage, p_at, lower, src_words, trg_words):
    correct = 0
    total = 0.0
    for i in src:
        trgs = [trg_words[x].lower() for x in src2trg[i]]
        for t in translation[i][:p_at]:
            # print(t, src2trg[i])
            if lower:
                # print(t)
                if trg_words[int(t)].lower() in trgs:
                    correct+=1
                    break               
            else:
                if int(t) in src2trg[i]:
                    correct+=1
                    break
        total +=1
    print('Coverage:{0:7.2%}  P@{2}:{1:7.2%}'.format(coverage, correct/total, p_at))
